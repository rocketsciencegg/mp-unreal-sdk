...
const FString& HttpMultipartFormData::GetBoundary() const
{
	if (Boundary.IsEmpty())
	{
		...
		// EGuidFormats::Short was added in UE 4.24, we replicate the functionality of ToString(EGuidFormats::String) here for compatibility reasons.
		FGuid Guid = FGuid::NewGuid();
		const uint32 Data[] = {Guid.A, Guid.B, Guid.C, Guid.D};
		...
		Boundary = Result;
#else // UE 4.24+ uses EGuidFormats::Short
...
void HttpMultipartFormData::SetupHttpRequest(const FHttpRequestRef& HttpRequest)
{
	...
	{
		UE_LOG(LogRSMultiplayGameServerSDK, Error, TEXT("Expected POST verb when using multipart form data"));
	}
...
void HttpMultipartFormData::AddFilePart(const TCHAR* Name, const HttpFileInput& File)
{
	...
	{
		UE_LOG(LogRSMultiplayGameServerSDK, Error, TEXT("Failed to load file (%s)"), *File.GetFilePath());
		return;
	}
